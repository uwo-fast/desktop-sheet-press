# 1 "F:\\FAST Research Group\\Heat Processes Projects\\Mini Press 1000\\Software\\MP_PI_Serial_v6\\MP_PI_Serial_v6.ino"
// Libraries for PID and Thermocouples
# 3 "F:\\FAST Research Group\\Heat Processes Projects\\Mini Press 1000\\Software\\MP_PI_Serial_v6\\MP_PI_Serial_v6.ino" 2
# 4 "F:\\FAST Research Group\\Heat Processes Projects\\Mini Press 1000\\Software\\MP_PI_Serial_v6\\MP_PI_Serial_v6.ino" 2
# 5 "F:\\FAST Research Group\\Heat Processes Projects\\Mini Press 1000\\Software\\MP_PI_Serial_v6\\MP_PI_Serial_v6.ino" 2
# 6 "F:\\FAST Research Group\\Heat Processes Projects\\Mini Press 1000\\Software\\MP_PI_Serial_v6\\MP_PI_Serial_v6.ino" 2

// For LCD

# 10 "F:\\FAST Research Group\\Heat Processes Projects\\Mini Press 1000\\Software\\MP_PI_Serial_v6\\MP_PI_Serial_v6.ino" 2


# 11 "F:\\FAST Research Group\\Heat Processes Projects\\Mini Press 1000\\Software\\MP_PI_Serial_v6\\MP_PI_Serial_v6.ino"
...
Adafruit_MAX31855 thermocouple1(6, 5, 8);
Adafruit_MAX31855 thermocouple2(6, 7, 8);
...
PID myPID1(&Input1, &Output1, &Setpoint, consKp, consKi, consKd, 0);
PID myPID2(&Input2, &Output2, &Setpoint, consKp, consKi, consKd, 0);
...
// For Rotary Encoder
# 81 "F:\\FAST Research Group\\Heat Processes Projects\\Mini Press 1000\\Software\\MP_PI_Serial_v6\\MP_PI_Serial_v6.ino" 2
# 82 "F:\\FAST Research Group\\Heat Processes Projects\\Mini Press 1000\\Software\\MP_PI_Serial_v6\\MP_PI_Serial_v6.ino" 2
...
void setup() {
  ...
  encoder = new ClickEncoder(3, 4, 2, 4);
  ...
  pinMode(11 /* 14 //A0*/, 0x1);
  pinMode(12 /* 15 //A1*/, 0x1);
  // Check SSR lights
  checkSSR(1);
  checkSSR(2);
...
void buttonUpdateGUI(char (*displayArray)[23], int menuLength, int &dispGUIIndex, int &cursGUIIndex, bool &encRotationFlag) {
  ...
  maxGUIIndex = j;
  cursGUIIndex = ((((cursGUIIndex)>(1)?(cursGUIIndex):(1)))<(maxGUIIndex)?(((cursGUIIndex)>(1)?(cursGUIIndex):(1))):(maxGUIIndex));
...
void variableUpdater(double change, char action){
  switch(action) {
    ...
    case 'a':
      ...
      setTemp = ((0)>(((350)<(setTemp)?(350):(setTemp)))?(0):(((350)<(setTemp)?(350):(setTemp)))); // setTemp Upper limit set here!
      break;

    case 'b':
      processDuration += (5 * change*1000*60); // *5 changes increase for knob click | *1000*60*60 scales the change to minutes
      processDuration = ((5*1000*60)>(((995*1000*60)<(processDuration)?(995*1000*60):(processDuration)))?(5*1000*60):(((995*1000*60)<(processDuration)?(995*1000*60):(processDuration)))); // setTemp Upper limit set here!
      break;
...
void processEncRotary(bool &encRotationFlag) {
  ...
  if (newEncValue != 0 && !chngValFlag) {
    ...
    if (newEncValue < 0) {
      ...
      cursGUIIndex = ((maxGUIIndex)<(cursGUIIndex + 1)?(maxGUIIndex):(cursGUIIndex + 1));
    } else {
      // Encoder was turned in a counter-clockwise direction
      cursGUIIndex = ((1)>(cursGUIIndex - 1)?(1):(cursGUIIndex - 1));
    }
  }
...
void processLoop() {
  ...
  while (!processDone) { // Loop continues until process is done
   ...
   processTimeTotal += processLoopInterval;
    ...
    if (currentMillis - lastMillis >= processLoopInterval) {
      ...
      slowPWM(11 /* 14 //A0*/, cycleStart1, period, dutyCycle1);
      slowPWM(12 /* 15 //A1*/, cycleStart2, period, dutyCycle2);
  ...
  digitalWrite(11 /* 14 //A0*/, 0x0);
  digitalWrite(12 /* 15 //A1*/, 0x0);
}
...
void checkIsnan(double& temp1, double& temp2) {
  ...
  errorFlagsMAX[0] = (e1 & (0x01) /*|< Enable open circuit fault check*/) ? 1 : 0;
  errorFlagsMAX[1] = (e1 & (0x02) /*|< Enable short to GND fault check*/) ? 1 : 0;
  errorFlagsMAX[2] = (e1 & (0x04) /*|< Enable short to VCC fault check*/) ? 1 : 0;
  // Read error flags from thermocouple 2 and generate error code
  uint8_t e2 = thermocouple2.readError();
  errorFlagsMAX[3] = (e2 & (0x01) /*|< Enable open circuit fault check*/) ? 1 : 0;
  errorFlagsMAX[4] = (e2 & (0x02) /*|< Enable short to GND fault check*/) ? 1 : 0;
  errorFlagsMAX[5] = (e2 & (0x04) /*|< Enable short to VCC fault check*/) ? 1 : 0;
...
void dynamicTuning() {
  double gap1 = ((Setpoint-Input1)>0?(Setpoint-Input1):-(Setpoint-Input1));
  ...
  double gap2 = ((Setpoint-Input2)>0?(Setpoint-Input2):-(Setpoint-Input2));
...
void initLCD_I2C(uint8_t SCL, uint8_t SDA)
{
  ...
  digitalWrite(SCL, 0x1);
  digitalWrite(SDA, 0x1);
